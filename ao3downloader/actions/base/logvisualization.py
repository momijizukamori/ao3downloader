...
class LogVisualizationAction(BaseAction):
    def action(self):
        ...
        if not os.path.exists(logfile):
            self.log(strings.INFO_NO_LOG_FILE)
            return
        ...
        with open(logfile, 'r', encoding='utf-8') as f:
            while True:
                ...
                start = self.get_timestamp(buf[0], filenumber)
                end = self.get_timestamp(buf[-1], filenumber)
                filename = visfile.format(f'{start}-{end}')

                if os.path.exists(filename):
                    continue # we already have this chunk

                self.log(strings.INFO_PARSING_LOGS.format(start, end))
